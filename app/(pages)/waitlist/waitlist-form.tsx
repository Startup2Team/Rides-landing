...
export function WaitlistForm() {
  ...
  // to opt out, not as friction every signup has to clear.
  const [consentLaunch, setConsentLaunch] = useState(true);
  const [consentMarketing, setConsentMarketing] = useState(false);

  const [turnstileToken, setTurnstileToken] = useState("");
  const [turnstileUnavailable, setTurnstileUnavailable] = useState(false);
  const [showTurnstileError, setShowTurnstileError] = useState(false);
  // Bumping this remounts <TurnstileWidget> from scratch — used by
  // resetForm() so "join another" gets a fresh widget/token instead of a
  // stale one left over from the previous submission.
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // rate limits, rather than turning a script hiccup into a dead end. When
    // the widget DID load, still require its (real) token before sending.
  ...
  if (state === "success" && receipt) {
    ...
    const whatsappHref = shareLink
      ...
      : null;
    // Only promise a contact method we can actually use — phone and email
    // are both optional, so someone who gave neither shouldn't be told
    // "we'll text you".
    const successBodyKey = phone.trim()
      ? "successBodyPhoneTemplate"
      : email.trim()
        ? "successBodyEmailTemplate"
        : "successBodyNoContactTemplate";
    ...
    return (
      <div className="rounded-2xl border border-primary/30 bg-primary/[0.04] p-8 backdrop-blur-sm">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {renderTemplate(t(successBodyKey), {
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} noValidate className="mt-8 space-y-6">
        ...
        <div className="space-y-3 pt-2">
          {/* Defaulted to checked (see useState above) and never required —
              joining the waitlist already implies wanting the launch
              notification, so this isn't gated on submit like the fields
              above. */}
        ...
        <div>
          ...
          {turnstileUnavailable ? (
            // Fail-open, not a dead end: the form still submits (with an
            // empty token, backstopped server-side by rate limits) — this is
            // just a heads-up, not a blocker, so no retry action here.
